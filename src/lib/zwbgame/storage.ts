...
const riderSchema = z.object({
  ...
  gels: z.number().int().min(0).max(3), bottles: z.number().int().min(0).max(3), fed: z.boolean(), eating: finite.min(0).max(5), drinking: finite.min(0).max(4), digesting: finite.min(0).max(90), sheltered: z.boolean(), finishTime: finite.min(0).max(900).nullable(), attacks: finite.min(0), shelteredSeconds: finite.min(0).max(901),
...
const savedSchema = z.object({
  ...
  config: z.object({ courseId: z.enum(["polder", "ardennen", "heuvelrug", "alpen"]), seed: z.number().int(), playerId: z.string() }),
  tick: z.number().int().min(0).max(4501), finished: z.boolean(), riders: z.array(riderSchema).min(1).max(24),
});
...
const resultSchema = z.object({ id: z.string(), courseId: z.enum(["polder", "ardennen", "heuvelrug", "alpen"]), date: z.string(), place: z.number().int().min(1).max(24), count: z.number().int().min(1).max(24), seconds: finite.min(0).max(1800) });
