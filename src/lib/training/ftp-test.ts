 ...
 * verwijderd.
 *
 * Het profiel volgt de nieuwste test. Tot 11 september 2026 alleen als het die
 * waarde ook echt had, zodat een ingetypt getal of een eFTP uit intervals.icu
 * bleef staan; sinds de afgesproken voorrang (test, dan intervals.icu, dan het
 * profiel) wijkt zo'n getal voor de test. Blijft er na een verwijdering geen
 * enkele test over, dan laten we het profiel staan — elk wattage in een schema
 * hangt eraan, dus het leeghalen is schadelijker dan een verouderd getal. Hing
 * het aan de verwijderde test, dan krijgt het lid dat te zien.
 */
export function profileFtpAfterChange(input: {
  ...
  if (latestFtpWatts == null) {
    ...
    return { ftpWatts: profileFtpWatts, changed: false, withoutTest: followed };
  }
  return {
...
export type FtpTestChange = {
  ...
  /**
   * Er is geen test meer en het profiel volgt intervals.icu: dan neemt de
   * eerstvolgende sync de FTP weer over. Met een test erbij gebeurt dat niet
   * meer (zie teams/_actions.ts).
   */
...
async function syncProfileToLatestTest(
  ...
  const latest = await latestFtpTestFtp(admin, profileId);
  const outcome = profileFtpAfterChange({
    profileFtpWatts: current,
    changedFtpWatts,
    latestFtpWatts: latest,
  });
  ...
  return {
    ...
    overwrittenByIntervals: latest == null && Boolean(profile?.auto_sync_physique),
  };
}
 ...
 *
 * Hoorde er een testworkout bij, dan gaat die terug naar 'planned'. Sinds
 * pickFtpTestState() (11 september 2026) vraagt de kaart al om een uitslag
 * zolang er geen meting bij de test hoort, ongeacht de status; het terugzetten
 * houdt de status alleen in lijn met wat er is gebeurd.
 */
