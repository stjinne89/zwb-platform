...
// De serie stap voor stap doorlopen, met een aftelling per hold. Aan het eind
// één afvink-actie.
//
...
export function SeriesRunner({
  ...
  // Aftellen. Het doorschakelen gebeurt ín de timeout, dus nooit synchroon
  // tijdens het effect zelf.
  useEffect(() => {
    if (phase !== "running" || paused || seconds === null) return;
    const timer = window.setTimeout(() => {
      if (seconds <= 1) advance();
      else setSeconds(seconds - 1);
    }, 1000);
    return () => window.clearTimeout(timer);
    // advance hangt alleen van index en steps af; die zitten in deze deps.
  ...
  return (
    <div className="space-y-3 rounded-lg border bg-card p-4">
      <div className="flex items-baseline justify-between gap-3">
        <h3 className="font-semibold">{step.item.exercise.title}</h3>
