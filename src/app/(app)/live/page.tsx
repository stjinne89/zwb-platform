...
export default async function LivePage() {
  ...
  const [{ data: sessionRows }, { data: positionRows }] = await Promise.all([
    supabase
      .from("live_sessions")
      .select(
        "id, profile_id, mode, status_text, external_track_url, started_at, last_seen_at, profiles(display_name)",
      )
      .is("ended_at", null)
      ...
      <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-muted-foreground">
            Zie welke ZWB&apos;ers nu rijden. Outdoor riders verschijnen op de
            kaart, externe LiveTrack-links openen bij Garmin of Wahoo, en indoor
            riders delen waar je kunt joinen.
          </p>
        </div>
        {mySession && <StopLiveButton sessionId={mySession.id} />}
      </header>

      {/* Mijn actieve sessie: position tracker draait alleen voor outdoor */}
      {mySession?.mode === "outdoor" && (
        <PositionTracker sessionId={mySession.id} profileId={user.id} />
      )}
        ...
        <div className="space-y-4">
          ...
          {mySession && (
            <section className="rounded-md border border-destructive/30 bg-destructive/5 p-4 text-sm">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {mySession.mode === "outdoor"
                  ? "GPS streamt actief. Houd app open + scherm aan."
