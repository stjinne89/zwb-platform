...
function PartMapping({ part }: { part: { id: string; title: string } }) {
  ...
  const [segments, setSegments] = useState<Array<{ segmentId: string; name: string }> | null>(null);
  ...
  return <div className="space-y-2">
    <Button variant="outline" disabled={pending} onClick={() => start(async () => {
      ...
      setGroups(r.groups); setEventId(r.eventId); setSegments(r.segments); setSegmentId(r.segmentId ?? ""); setMessage("");
    ...
    {groups && <><div className="grid gap-2 sm:grid-cols-2">{groups.map((g) => <label key={g.id} className="text-sm">{g.label} · {g.name}
      ...
      {segments && (segments.length ? <label className="block text-sm">Sprintsegment
        <select className="ml-2 rounded border bg-background p-2" value={segmentId} onChange={(e) => setSegmentId(e.target.value)}>
          <option value="">Kies segment</option>{segments.map((s) => <option key={s.segmentId} value={s.segmentId}>{s.name}</option>)}
        </select></label> : <p className="text-sm">Route onbekend: geen segmenten.</p>)}
      <Button disabled={pending || groups.some((g) => !g.league)} onClick={() => start(async () => {
        const r = await saveZwiftGroups(part.id, eventId, Object.fromEntries(groups.map((g) => [g.id, g.league!])), segments ? segmentId || null : undefined);
